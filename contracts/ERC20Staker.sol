...
contract ERC20Staker {
  ...
  event Staked(address indexed _staker, uint256 amount);
  event Withdrawn(address indexed _staker, uint256 amount);
  event RewardPaid(address indexed _staker, uint256 amount);
  ...
  function stake(uint256 _amount) external {
    ...
    require(tokenToStake.balanceOf(msg.sender) >= _amount, "Insufficient balance");
    require(tokenToStake.allowance(msg.sender, address(this)) >= _amount, "Insufficient allowance");
